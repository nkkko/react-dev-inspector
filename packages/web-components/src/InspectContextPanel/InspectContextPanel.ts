...
export class InspectContextPanel<Item extends ItemInfo = ItemInfo> {
  ...
  public show(params: InspectContextPanelShowParams<Item> & {
    onClickOutside?: () => void;
  }) {
  ...
  private listenClickOutside = () => {
    if (this.#clickOutsideSubscription && !this.#clickOutsideSubscription.closed) {
      return
    }

    this.#clickOutsideSubscription = fromEvent<MouseEvent>(window, 'pointerdown', { capture: true }).pipe(
